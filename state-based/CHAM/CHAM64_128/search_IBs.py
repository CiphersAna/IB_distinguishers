...
def format_out(hmk, tmk, hrkey, trkey, hinit_state, hfini_state, tinit_state, tfini_state):
    ...
    statement += "hmk:\n"
    for i in range(0, 8):
        statement += "{}\n".format(hmk[(16 * i) : (16 * i + 16)])
    statement += "hrkey:\n"
    for k in hrkey:
        statement += "{}\n".format(k)
    statement += "tmk:\n"
    for i in range(0, 8):
        statement += "{}\n".format(tmk[(16 * i) : (16 * i + 16)])
    statement += "trkey:\n"
    for k in trkey:
        statement += "{}\n".format(k)
    statement += "hinit_state:\n"
    statement += "{}\n".format(hinit_state)
...
def search_IBs():
    ...
    while True:
        ...
        while True:
            ...
            solve_file = "{}/{}".format(dir1, "model_r{}.stp".format(search_round))
            result_file = "{}/{}".format(dir1, "model_r{}.txt".format(search_round))
