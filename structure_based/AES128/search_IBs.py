...
def determine_IB(r_begin, r_end, b, e, bk, ek, solve_file, thread_num):
    build_model.build_model2file(r_begin, r_end, 0, b, e, bk, ek, solve_file)
    flag = build_model.call_solver(thread_num, solve_file, r_begin, r_end, 0)
    return flag

def __search_IBs_process(share_dict, search_space, search_round, process_index, thread_num):

    dir0 = "Results"
    dir1 = "Results/AES_{}".format(search_round)
...
def search_IBs_process(search_round):
    params = search_params.search_param_trunc1word()
    process_num = 32
    ...
    with Manager() as manager:
        ...
        result = "Results/AES_{}/result.txt".format(search_round)
        f = open(result, "w")
