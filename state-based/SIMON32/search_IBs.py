...
def __search_IBs_process(share_dict, search_space, search_round, process_index, thread_num):
    ...
    dir0 = "Results"
    dir1 = "Results/{}_{}".format(config_search.name, search_round)
    ...
    for each_search in search_space:
        ...
        bk = [each_search[4], each_search[5]]
        ek = []
        t1 = time.time()
        flag = determine_IB(0, search_round, b, e, bk, ek, solve_file, thread_num)
        t2 = time.time()
...
def search_IBs_process(search_round):
    params = search_params.search_param_bit()
    process_num = 16
    ...
    with Manager() as manager:
        ...
        result = "Results/{}_{}/result.txt".format(config_search.name, search_round)
        f = open(result, "w")
        f.write("Cost {}, find {} {}-round IBs\n".format(tt2 - tt1, total_distinguisher_num, search_round))
        f.close()

if __name__ == "__main__":
    search_IBs_process(9)
    search_IBs_process(10)
