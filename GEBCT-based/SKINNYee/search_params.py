...
def v_forward():
    ...
    for v in range(0, 65536):
        x0 = [((v >> i) & 0x1) for i in range(0, 16)]
        x1 = lfsr_forward(lfsr_forward(x0))
        x2 = lfsr_forward(lfsr_forward(x1))
        x3 = lfsr_forward(lfsr_forward(x2))
        if check_zero(x0) and check_zero(x1) and check_zero(x2):
            s.append(x3)
    return s
...
def lfsr_backward(x):
    ...
    xx3 = [x3[2], x3[3], x3[0] ^ x3[3], x3[0] ^ x3[1] ^ x3[3]]
...
def v_backward():
    ...
    for v in range(0, 65536):
        x0 = [((v >> i) & 0x1) for i in range(0, 16)]
        x1 = lfsr_backward(lfsr_backward(x0))
        x2 = lfsr_backward(lfsr_backward(x1))
        x3 = lfsr_backward(lfsr_backward(x2))
        if check_zero(x0) and check_zero(x1) and check_zero(x2):
            s.append(x3)
    return s
...
def derive_rk(x):
    ...
    for i in range(0, 2):
        for j in range(0, 4):
            if len(x[i][j]) != 0:
                for k in range(0, 4):
                    y[i][j][k] = x[i][j][k] ^ x[i][j][k + 4] ^ x[i][j][k + 8] ^ x[i][j][k + 12]
    return y
