...
def determine_IB(total_round, b, e, solve_file, thread_num):
    build_model.build_model2file(total_round, b, e, solve_file)
    return build_model.call_solver(thread_num, solve_file, total_round)

def __search_IBs_process(share_dict, search_space, search_round, process_index, thread_num):

    dir0 = "Results"
    dir1 = "Results/GIFT64_{}".format(search_round)
    ...
    for each_search in search_space:
        search_count += 1
        b = [each_search[0], each_search[1]]
        e = [each_search[2], each_search[3]]
        t1 = time.time()
        flag = determine_IB(search_round, b, e, solve_file, thread_num)
        t2 = time.time()
...
def search_IBs_process(search_round):
    ...
    with Manager() as manager:
        ...
        result = "Results/GIFT64_{}/result.txt".format(search_round)
        f = open(result, "w")
        f.write("Cost {}, find {} {}-round T3-IBDs\n".format(tt2 - tt1, total_distinguisher_num, search_round))
        f.close()

if __name__ == "__main__":
    search_IBs_process(5)
