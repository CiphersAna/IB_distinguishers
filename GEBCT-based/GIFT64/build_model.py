...
def sp_mfold(x):
    ...
    for i in range(0, 16):
        yy = []
        for j in range(0, 4):
            yy.append(x[j][(4 * i) : (4 * i + 4)])
        y.append(yy)
    return y

def sbox_layer_mfold(x, y, tx, ty, m):
    statement = []
    xx = sp_mfold(x)
    yy = sp_mfold(y)
    txx = sp_mfold(tx)
    tyy = sp_mfold(ty)
    for i in range(0, 16):
        statement += sbox_gebct(xx[i], yy[i], txx[i], tyy[i])
    return statement
...
def set_diff_in(x):
    ...
    for j in range(0, len(x[0])):
        statement.append("{} = 0bin0".format(x[0][j]))
    for j in range(0, len(x[0])):
        statement.append("{} = 0bin0".format(x[2][j]))
    return statement

def set_diff_out(x):
    statement = []
    for j in range(0, len(x[0])):
        statement.append("{} = 0bin0".format(x[1][j]))
    for j in range(0, len(x[0])):
        statement.append("{} = 0bin0".format(x[3][j]))
    return statement
...
def set_diff_kin(k, b):
    ...
    for i in range(0, 32):
        statement.append("{} = 0bin{}".format(rk0[i], b[0][i]))
        statement.append("{} = 0bin{}".format(rk2[i], b[1][i]))

    for i in range(0, 96):
        statement.append("{} = 0bin0".format(nrk0[i]))
        statement.append("{} = 0bin0".format(nrk2[i]))
    return statement
...
def set_diff_kout(k, e):
    ...
    for i in range(0, 32):
        statement.append("{} = 0bin{}".format(rk1[i], e[0][i]))
        statement.append("{} = 0bin{}".format(rk3[i], e[1][i]))

    for i in range(0, 96):
        statement.append("{} = 0bin0".format(nrk1[i]))
        statement.append("{} = 0bin0".format(nrk3[i]))
    return statement
...
def body(total_round, b, e):
    ...
    for r in range(0, total_round):
        ...
        ty = var_declare_mfold("ty", r, 64, 4)
        tx = var_declare_mfold("tx", r, 64, 4)
        all_var.append(copy.deepcopy(tx))
        all_var.append(copy.deepcopy(ty))

        statement += sbox_layer_mfold(y, px1, tx, ty,4)
...
def bit2nibble(x):
    y = []
    for i in range(0, 4):
        y.append((x[i][3] << 3) ^ (x[i][2] << 2) ^ (x[i][1] << 1) ^ (x[i][0] << 0))
    return y

def verify_gebct(x1, y1, tx1, ty1):
    x = bit2nibble(x1)
    y = bit2nibble(y1)
    tx = bit2nibble(tx1)
    ty = bit2nibble(ty1)
...
def verify_sbox_layer_mfold(x, y, tx, ty, m):
    flag = True
    xx = sp_mfold(x)
    yy = sp_mfold(y)
    txx = sp_mfold(tx)
    tyy = sp_mfold(ty)
    for i in range(0, 16):
        if not verify_gebct(xx[i], yy[i], txx[i], tyy[i]):
            flag = False
            break
    return flag
...
def verify_solution(res, total_round):
    ...
    for r in range(0, total_round):
        ...
        tx = get_value(var_declare_mfold("tx", r, 64, 4), value_dict)
        ty = get_value(var_declare_mfold("ty", r, 64, 4), value_dict)
        flag = verify_sbox_layer_mfold(y, px1, tx, ty, 4)
        assert(flag)
