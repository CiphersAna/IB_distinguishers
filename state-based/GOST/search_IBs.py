...
def __search_IB_process(share_dict, search_space, search_round, process_index, thread_num):
    ...
    dir0 = "Results"
    dir1 = "Results/GOST_{}".format(search_round)
    ...
    for each_search in search_space:
        ...
        e = each_search[1]
        t1 = time.time()
        flag = determine_IB(24, 24 + search_round, b, e, solve_file, thread_num)
        t2 = time.time()
        print(search_count, total_search_count, flag, each_search, t2 - t1)
        if flag:
...
def search_IB_process():
    ...
    with Manager() as manager:
        ...
        for i in range(0, process_num):
            ...
            p = Process(target=__search_IB_process, args=(shared_dict, search_space_process, search_round, i, 1))
            p.start()
            processes.append(p)
        ...
        result = "Results/GOST_{}/result.txt".format(search_round)
        f = open(result, "w")
        f.write("Cost {}, find {} {}-round T5-IBDs\n".format(tt2 - tt1, total_distinguisher_num, search_round))
        f.close()
