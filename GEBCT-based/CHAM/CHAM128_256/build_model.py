...
def body(r_begin, r_end, b, e, bk, ek):
    ...
    for r in range(r_begin, r_end):
        ...
        tx = var_declare_mfold("tx", r, 32, 4)
        ty = var_declare_mfold("ty", r, 32, 4)
        tz = var_declare_mfold("tz", r, 32, 4)
        all_var += [tx, ty, tz]
        ...
        if ((r % 2) == 0):
            ...
            statement += add_layer_gebct(x, u, v, tx, ty, tz)
...
def verify_solution(res, r_begin, r_end):
    ...
    for r in range(r_begin, r_end):
        ...
        tx = get_value(var_declare_mfold("tx", r, 32, 4), value_dict)
        ty = get_value(var_declare_mfold("ty", r, 32, 4), value_dict)
        tz = get_value(var_declare_mfold("tz", r, 32, 4), value_dict)
        flag = verify_gebct(x, u, v, tx, ty, tz)
        assert(flag)
