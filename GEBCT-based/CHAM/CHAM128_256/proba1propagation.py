...
def body(r_begin, r_end):
    ...
    mk = var_declare("mk", 0, 256)
    all_var.append(copy.deepcopy(mk))
    rk = var_declare("rk", 0, 512)
    all_var.append(copy.deepcopy(rk))

    statement1, rkey = round_key(copy.deepcopy(rk), copy.deepcopy(mk))
    statement += statement1

    x = var_declare("x", r_begin, 32)
    y = var_declare("y", r_begin, 32)
    z = var_declare("z", r_begin, 32)
    w = var_declare("w", r_begin, 32)
    all_var += [x, y, z, w]
    statement += init_restrict(x + y + z + w + mk)

    for r in range(r_begin, r_end):
        x1 = var_declare("x", r + 1, 32)
        y1 = var_declare("y", r + 1, 32)
        z1 = var_declare("z", r + 1, 32)
        w1 = var_declare("w", r + 1, 32)
        all_var += [x1, y1, z1, w1]

        u = var_declare("u", r, 32)
        v = var_declare("v", r, 32)
        all_var += [u, v]
...
def verify_solution(res, r_begin, r_end):
    ...
    mk = get_value(var_declare("mk", 0, 256), value_dict)

    rk = get_value(var_declare("rk", 0, 512), value_dict)
    ...
    x = get_value(var_declare("x", r_begin, 32), value_dict)
    y = get_value(var_declare("y", r_begin, 32), value_dict)
    z = get_value(var_declare("z", r_begin, 32), value_dict)
    w = get_value(var_declare("w", r_begin, 32), value_dict)
    ...
    for r in range(r_begin, r_end):
        u = get_value(var_declare("u", r, 32), value_dict)
        v = get_value(var_declare("v", r, 32), value_dict)

        x1 = get_value(var_declare("x", r + 1, 32), value_dict)
        y1 = get_value(var_declare("y", r + 1, 32), value_dict)
        z1 = get_value(var_declare("z", r + 1, 32), value_dict)
        w1 = get_value(var_declare("w", r + 1, 32), value_dict)
...
def search_proba1_header(r_begin, r_end, add_statement):
    dir0 = "Results"
    dir1 = "Results/CHAM128_256_proba1_header_{}_{}".format(r_begin, r_end)
...
def search_proba1_trailer(r_begin, r_end, add_statement):
    dir0 = "Results"
    dir1 = "Results/CHAM128_256_proba1_trailer_{}_{}".format(r_begin, r_end)
    if not os.path.exists(dir0):
        os.makedirs(dir0)
    if not os.path.exists(dir1):
        os.makedirs(dir1)
