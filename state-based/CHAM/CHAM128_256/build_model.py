...
def round_key(rk, mk):
    ...
    for i in range(0, 8):
        statement += rk1gen(rk[(32 * i) : (32 * i + 32)], mk[(32 * i) : (32 * i + 32)])
        rkey[i] = copy.deepcopy(rk[(32 * i) : (32 * i + 32)])
        j = (i + 8) ^ 1
        statement += rk2gen(rk[(32 * j) : (32 * j + 32)], mk[(32 * i) : (32 * i + 32)])
        rkey[j] = copy.deepcopy(rk[(32 * j) : (32 * j + 32)])
...
def set_diff_in(x, b):
    ...
    for i in range(0, 128):
        statement.append("BVXOR({}, {}) = 0bin{}".format(x[0][i], x[1][i], b[i]))
    for i in range(0, 128):
        statement.append("BVXOR({}, {}) = 0bin{}".format(x[2][i], x[3][i], b[i]))
    return statement

def set_diff_out(x, e):
    statement = []
    for i in range(0, 128):
        statement.append("BVXOR({}, {}) = 0bin{}".format(x[1][i], x[2][i], e[i]))
    for i in range(0, 128):
        statement.append("BVXOR({}, {}) = 0bin{}".format(x[0][i], x[3][i], e[i]))
    return statement

def set_diff_kin(kk, b):
    statement = []
    for i in range(0, 256):
        statement.append("BVXOR({}, {}) = 0bin{}".format(kk[0][i], kk[1][i], b[i]))
    for i in range(0, 256):
        statement.append("BVXOR({}, {}) = 0bin{}".format(kk[2][i], kk[3][i], b[i]))
    return statement

def set_diff_kout(kk, e):
    statement = []
    for i in range(0, 256):
        statement.append("BVXOR({}, {}) = 0bin{}".format(kk[1][i], kk[2][i], e[i]))
    for i in range(0, 256):
        statement.append("BVXOR({}, {}) = 0bin{}".format(kk[0][i], kk[3][i], e[i]))
    return statement
...
def body(r_begin, r_end, b, e, bk, ek):
    ...
    mk = var_declare_mfold("mk", 0, 256, 4)
    all_var.append(copy.deepcopy(mk))
    rk = var_declare_mfold("rk", 0, 512, 4)
    all_var.append(copy.deepcopy(rk))

    statement1, rkey = round_key_mfold(copy.deepcopy(rk), copy.deepcopy(mk), 4)
    statement += statement1

    x = var_declare_mfold("x", r_begin, 32, 4)
    y = var_declare_mfold("y", r_begin, 32, 4)
    z = var_declare_mfold("z", r_begin, 32, 4)
    w = var_declare_mfold("w", r_begin, 32, 4)
    all_var += [x, y, z, w]
    ...
    for r in range(r_begin, r_end):
        ...
        z1 = var_declare_mfold("z", r + 1, 32, 4)
        w1 = var_declare_mfold("w", r + 1, 32, 4)
        all_var += [x1, y1, z1, w1]

        u = var_declare_mfold("u", r, 32, 4)
        v = var_declare_mfold("v", r, 32, 4)
        p = var_declare_mfold("p", r, 32, 4)
        all_var += [u, v, p]
        ...
        if ((r % 2) == 0):
            ...
            statement += xor_layer_mfold(x, round_constant(r), p, 4)
            statement += add_layer_mfold(p, u, v, 4)
...
def verify_solution(res, r_begin, r_end):
    ...
    rkey = [[] for i in range(0, 16)]
    rk = get_value(var_declare_mfold("rk", 0, 512, 4), value_dict)
    for i in range(0, 8):
        ek = []
        for j in range(0, 4):
            ek.append(rk[j][(32 * i) : (32 * i + 323)])
        rkey[i] = ek
        k = (i + 8) ^ 1
        ek = []
        for j in range(0, 4):
            ek.append(rk[j][(32 * k) : (32 * k + 32)])
        rkey[k] = ek

    x = get_value(var_declare_mfold("x", r_begin, 32, 4), value_dict)
    y = get_value(var_declare_mfold("y", r_begin, 32, 4), value_dict)
    z = get_value(var_declare_mfold("z", r_begin, 32, 4), value_dict)
    w = get_value(var_declare_mfold("w", r_begin, 32, 4), value_dict)

    for r in range(r_begin, r_end):
        x1 = get_value(var_declare_mfold("x", r + 1, 32, 4), value_dict)
        y1 = get_value(var_declare_mfold("y", r + 1, 32, 4), value_dict)
        z1 = get_value(var_declare_mfold("z", r + 1, 32, 4), value_dict)
        w1 = get_value(var_declare_mfold("w", r + 1, 32, 4), value_dict)

        u = get_value(var_declare_mfold("u", r, 32, 4), value_dict)
        v = get_value(var_declare_mfold("v", r, 32, 4), value_dict)
        p = get_value(var_declare_mfold("p", r, 32, 4), value_dict)
        ...
        if ((r % 2) == 0):
            ...
            assert(flag)
            flag = verify_xor_const_mfold(x, p, r, 4)
            assert(flag)
            flag = verify_add_mfold(p, u, v, 4)
            assert(flag)
